...
const UpdateTransaction = () => {
  ...
  if (isTransactionLoading || isCategoriesLoading) {
    return <ClipLoader color="green" loading={true} size={30} />;
  }
  ...
  return (
    ...
    >
      <div className="text-center">
        ...
        <p className="text-gray-600">Fill in the details to update.</p>
      </div>
      {/* Display alert message */}
      {isAddTxnErr && (
        <AlertMessage
          type="error"
          message={txnErr?.message || "Something went wrong. Please try again."}
        />
      )}
      {isSuccess && (
        <AlertMessage
      ...
      {isPending && (
        <div className="flex justify-center mt-4">
          <ClipLoader color="green" loading={true} size={30} />
          <span className="ml-2 text-red-500">Updating transaction...</span>
        </div>
      )}
